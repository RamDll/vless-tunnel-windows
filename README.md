...
* `doctor` снимает зависшие WFP-фильтры и осиротевшие маршруты, предупреждает о сбитых часах (ломают REALITY/TLS);
...
1. Скачайте с [Releases](https://github.com/RamDll/vless-tunnel-windows/releases)
   четыре файла: `vless-tunnel-setup.exe`, `install.cmd`, `trust.ps1`,
   `vless-tunnel.cer` — положите рядом, в одну папку.
2. Дважды кликните `install.cmd`. Windows спросит подтверждение через
   UAC («Разрешить этому приложению вносить изменения…») — после «Да»
   `install.cmd` сам вызовет `trust.ps1`: тот сверит отпечаток
   `vless-tunnel.cer` с зашитым в себя, поставит сертификат в доверенные
   хранилища Windows и запустит установщик — без этого шага Windows
   показала бы «Неизвестный издатель» (сертификат самоподписанный, не
   от публичного удостоверяющего центра).
...
`C:\Program Files\vless-tunnel\unins000.exe`: сам явно выключает
активный туннель и закрывает трей перед остановкой службы, спросит,
удалять ли сохранённую ссылку на сервер (при тихом удалении,
`/VERYSILENT`, — оставит её), и снимает службу, маршруты и WFP-фильтры
до конца, даже если сама остановка службы не уложилась в таймаут.
...
"C:\Program Files\vless-tunnel\VlessTunnel.Cli.exe" doctor          # снять зависшие фильтры и маршруты, проверить часы
```
